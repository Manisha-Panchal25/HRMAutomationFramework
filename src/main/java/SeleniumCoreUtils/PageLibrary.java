...
public class PageLibrary extends seleniumCore {
  ...
  public void verifyElementAttributeContains(By locator, String attribute, String expectedValue){
	  WebElement elementName = findFreshElement(locator);
...
//      wait.waitForElementToBeVisible(elementName);  
  ...
  public void verifyElementCssContains(By locator, String cssProperty, String expectedValue){
      ...
      wait.waitForElementToBeVisible(elementName);  
      String actualValue = elementName.getCssValue(cssProperty);
      logMessage("Expected value: "+expectedValue);
      logMessage("Actual value: "+actualValue);
      Assert.assertEquals(actualValue, expectedValue, "❌ ASSERT FAILED: element '" + elementName.getCssValue(cssProperty)
      + "' css property is not as expected: ");
      logMessage("✔ ASSERT PASSED: element " + elementName.getCssValue(cssProperty)
              + " is visible and css property value is " + expectedValue);
  }
...
//	      clickUsingActionScript(element);
  ...
  public void enterTextWithReturnKey(By locator, String key) {
	  	WebElement element = finder(locator);
	      wait.waitForElementToBeClickable(element);
	      
	      element.clear();
	      element.sendKeys(key, Keys.RETURN);
	      logMessage("Used key " + key + "  in the "+element.getAttribute("id")+" element");
   ...
   */
   ...
   System.out.println(driver.getWindowHandles().size());
    ...
    public boolean isElementDisplayed(By locator)
        ...
        removehighlightElement(elementName);
        return result;
    }
    ...
    public boolean isElementEnabled(WebElement element) {
        try {
            wait.waitForElementToBeVisible(element); // Ensure element is visible
            // Check if the element has a 'disabled' attribute or check CSS class
            boolean isDisabled = (Boolean) ((JavascriptExecutor) driver).executeScript(
                "return arguments[0].hasAttribute('disabled')", element);

            // Alternatively, check for a specific CSS class indicating a disabled state
            // String classAttribute = element.getAttribute("class");
            // boolean isDisabled = classAttribute != null && classAttribute.contains("disabled");

            boolean isEnabled = !isDisabled;
            logMessage("✔ Element " + element + " is enabled: " + isEnabled);
            return isEnabled;
        } catch (Exception e) {
            logMessage("❌ Error checking if element is enabled: " + e.getMessage());
            return false;
        }
    }
    ...
    public void hoverClick(By parentElement, By childElement) {
    	Actions actions = new Actions(driver);
    	WebElement parent = finder(parentElement);
    	actions.moveToElement(parent).pause(100);

    	WebElement child = finder(childElement);
    ...
    public String getDate()
	   ...
	   public void clearKeysUsingActionBuilder(By locator) {
		   Actions actions = new Actions(driver);

		   actions.click(finder(locator))
     ...
     public void getLogs()
     {
    	 LogEntries logs = driver.manage().logs().get(LogType.BROWSER);
			for (LogEntry entry : logs) {
				System.out.println("Browser Console Error: " + entry.getMessage());
